...
def CreateHeaderDict():
	...
	wb = xlrd.open_workbook(PATH + '/data/Sequence_Number_and_Table_Number_Lookup.xls')
	...
	for row in range(1, sheet.nrows):
		...
		# read in the rest of the line and update the hierarchy based on identifiers in the file
		...
		# Below are rules to identify the hierarchy for each line in the header file
		if subjectarea != '':
			lvl1 = subjectarea
			lvl2 = title
			lvl3 = ''
...
def ReadData(H, G):
	...
	# TODO this is MA specific
	DATADIR = 'data/Massachusetts_Tracts_Block_Groups_Only/'
	filelist = [file for file in os.listdir(PATH + DATADIR) if file.lower().endswith('.txt') & file.lower().startswith('e')]

	
	# Read in each file and get the column head for each row
	for efile in filelist[1:10]:
		
		mfile = 'm' + efile[1:]
		
		#print efile
		#fid = open(PATH + DATADIR + efile, 'rU')
		#mid = open(PATH + DATADIR + mfile, 'rU')
		...
		# opening both at once to grab the margin too
		e = csv.reader(open(PATH + DATADIR + efile, 'rU'))
		m = csv.reader(open(PATH + DATADIR + mfile, 'rU'))
		...
		for erow in e:
			...
			if seq != '':
				...
				if len(erow) == H[seq]['NumHeaders'] + 6:
					1 == 1
				else:
					1 == 1
					print('ERROR! NUMBER OF COLUMNS DOESNT MATCH NUMBER OF HEADERS!' + '\t' + str(len(erow)) + '\t' + str(H[seq]['NumHeaders']))
					print('\n' + seq + '\t' + efile)
			...
			for i in range(len(headers)):
				...
				entry['geo'] = geo
				entry['time'] = erow[1]
				entry['title'] = headers[i]  
				entry['estimate'] = records[i]
				entry['margin'] = margins[i] 
				coll.insert(entry)
				print entry
